...
public class AttachController extends BizController {
	...
	public void upload(MultipartFile uploadFile, Integer objType, Long objId,
		...
		if (!checkFileType(fileType)) {
			...
			params.put("message", "不支持的文件格式，目前支持的格式：" + UPLOAD_SUPPORT);
		...
		try {
			String uploadSubPath = request.getSession().getServletContext()
					.getRealPath("/upload");
			String fileFullPath = uploadSubPath+"/"+diskName+"."+fileType;
			...
			try {
				...
				params.put("success", false);
				params.put("message", "文件上传失败，请联系系统管理员");
			} finally{
				if(out!=null){
					out.close();
				}
	...
	public void download(long attachId, HttpServletRequest request,
		...
		// TODO
		IcAttach attach = attachService.queryAttachList(new long[]{attachId}).get(0);
		String savePath = attach.getAttachPath();
		BufferedInputStream in = null;
		try {
			in = new BufferedInputStream(new FileInputStream(new File(savePath)));
			response.reset();
			response.setContentType("application/octet-stream");
			response.addHeader("Content-Disposition",
					"attachment;filename="
							+ new String(attach.getAttachName()
